const employees = [
  ...
  function findEmployeeBySpecialization(employeeSpecialization) {
    ...
    if (foundEmployee) {
      document.getElementById('employeeDetails').innerHTML =
        `<p>${foundEmployee.specialization}: ${foundEmployee.name} - ${foundEmployee.department} - $${foundEmployee.salary}</p>`;
    } else {
      document.getElementById('employeeDetails').innerHTML = 'No employee found with this specialization';
    }
  }